class Reports::DeliveriesController < Admin::AdminController
  ...
  def export_excel
    ...
    headers['Content-Disposition'] = 'attachment; filename="'+@business_name+' Delivery Report-' + Time.now.strftime("%B %d, %Y").to_s + '.xls"'
  ...
  def get_deliveries_data
      ...
      where = "select st.id, st.transaction_type, st.delivery_id, st.employee_id, st.product_id, st.created_at from stock_transactions st"
      if session[:terminal] != ''
