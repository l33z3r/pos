...
//    initCasioPrint();
    ...
    printCasioReceipt("\u001b|*14dLp0,300d1w1s2c1");
    printCasioReceipt("\u001b|P");


//    setCasioIndent(0)
//    printCasioReceipt("Coca Cola");
//    setCasioIndent(15)
//    printCasioReceipt("3.25");
//    printCasioClose();
...
function fetchCasioFinalReceiptHTML(includeBusinessInfo, includeServerAddedText, includeVatBreakdown) {
    ...
    allCasioOrderItemsRecptHTML = ""
    finalCasioReceiptHTML = "";


//    if(includeBusinessInfo) {
//        finalCasioReceiptHTML += fetchCasioBusinessInfoHeaderHTML();
//    }
...
//    if(includeBusinessInfo) {
        finalCasioReceiptHTML += fetchCasioFinalReceiptHeaderHTML();
//    }
    finalCasioReceiptHTML += "\n";
    finalCasioReceiptHTML += "\n";
    allCasioOrderItemsRecptHTML += getCasioOrderItemsReceiptHTML(totalOrder, false, false, false);

    finalCasioReceiptHTML += allCasioOrderItemsRecptHTML;
//
    finalCasioReceiptHTML += "\n";
    ...
    if(totalOrder.discount_percent) {
        ...
        finalCasioReceiptHTML += "\u001b|2C" + "Sub-Total: " + "\u001b|1C" +  subTotal + "\n";

        //calculate the amount of discount
        discountAmount = (totalOrder.pre_discount_price * totalOrder.discount_percent)/100;
        finalCasioReceiptHTML += "\u001b|2C" + "Discount " + "\u001b|1C" + totalOrder.discount_percent + "%" + discountAmount + "\n";
    } else {
        subTotal = totalOrder.total;
        finalCasioReceiptHTML += "\u001b|2C" + "Sub-Total: " + "\u001b|1C" + currency(subTotal, false) + "\n";
        discountAmount = 0;
    }

    finalCasioReceiptHTML += taxChargable ? fetchCasioTotalsHTMLWithTaxChargable() : fetchCasioTotalsWithoutTaxChargableHTML();

    if(typeof(totalOrder.split_payments) != 'undefined') {
        for(var pm in totalOrder.split_payments) {
            finalCasioReceiptHTML += "Paid: " + pm + currency(totalOrder.split_payments[pm]) + "\n";
        }
    } else if(cashTendered > 0) {
        finalCasioReceiptHTML += "Paid: " + currency(cashTendered) + "\n";
    }
...
function fetchCasioBusinessInfoHeaderHTML() {
    businessCasioInfoHeaderHTML = "";
    businessCasioInfoHeaderHTML += businessInfoMessage;
    businessCasioInfoHeaderHTML += "\n";

    return businessCasioInfoHeaderHTML;
}
...
function fetchCasioFinalReceiptHeaderHTML() {
    var headerHTML = "";

    var server = firstServerNickname(totalOrder);

    if(server) {
        headerHTML += "\u001b|2C" + "Server:  " + "\u001b|1C" + server +"\n";
    }
    ...
    headerHTML += "\u001b|2C" + "Time:  " + "\u001b|1C" + timestamp+"\n";

    if(totalOrder.table) {
        headerHTML += "Table:  " + totalOrder.table+"\n";
    }

    if(totalOrder.terminal_id) {
        headerHTML += "\u001b|2C" + "Terminal:  " + "\u001b|1C" + totalOrder.terminal_id+"\n";
    } else {
        headerHTML += "\u001b|2C" + "Terminal:  " + "\u001b|1C" + terminalID +"\n";
    }

    var orderNum = totalOrder.order_num;

    if(typeof(orderNum) != 'undefined') {
        headerHTML += "Order Number:  " +"\n";
    }
...
function fetchCasioTotalsHTMLWithTaxChargable() {
    //write the tax total
    taxAmount = ((subTotal - discountAmount) * globalTaxRate)/100;
    totalsHTML = taxLabel + " " + globalTaxRate + currency(taxAmount);
    ...
    totalsHTML += "\u001b|2C" + "Total: " + "\u001b|1C" + currency(total + totalOrder.cashback, false) + "\n";

    return totalsHTML;
}

function fetchCasioTotalsWithoutTaxChargableHTML() {
    totalsHTML = fetchServiceChargeHTML();
