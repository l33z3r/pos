...
function syncTableOrderFail(){orderInProcess=false;if(inTransferOrderItemMode){$('#tables_screen_status_message').hide();hideNiceAlert();transferOrderItemInProgress=false;inTransferOrderItemMode=false;showMenuScreen();niceAlert("Error, Server may be down. You may need to hit order again for both tables to ensure that the changes are system wide.");}else if(inSplitBillMode){cancelSplitBillMode();tableSelectMenu.setValue(tempSplitBillTableNum);doSelectTable(tempSplitBillTableNum);niceAlert("Error, Server may be down. You may need to hit order again for the original table to ensure that the changes are system wide.");$('#split_bill_select_item').show();}else{niceAlert("Order not sent, no connection, please try again");}}
...
function cacheUpdateCheckPoll(){if(appOnline&&!cacheDownloading){console.log("Checking for cache update");window.applicationCache.update();}
...
transferOrderInProgress=true;$('#nav_back_link').unbind();$('#nav_back_link').click(function(){niceAlert("Transfer table order in progress, please wait.");return;});showLoadingDiv("Transfer in progress, Please wait...");$.ajax({type:'POST',url:'/transfer_order',error:transferOrderError,data:{table_from_id:tableFrom,table_from_order_num:getCurrentOrder().order_num,table_to_id:tableTo}});}
...
function regenerateTerminalFingerprintCookie(){setRawCookie(terminalFingerPrintCookieName,"",-365);setFingerPrintCookie()
goTo("/");}
...
if(cacheDownloading){cacheDownloadingPopup();return false;}
return true;}
var pingTimeoutSeconds=60;var pingTimeoutMillis=pingTimeoutSeconds*1000;function pingHome(){$.ajax({url:"/ping",type:"GET",timeout:pingTimeoutMillis,success:function(){setConnectionStatus(true);},error:function(){setConnectionStatus(false);}});}
