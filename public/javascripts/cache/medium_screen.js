...
menuSelectMenu.setValue(selectedDisplayId);showLoadingDiv("Downloading Orders. Please Wait.");callHomePoll();clueyScheduler();}
...
function getAllOrderItemsReceiptHTML(order,includeNonSyncedStyling,includeOnClick,includeServerAddedText,groupItems){if(groupItems){orderItems=groupOrderItems(order);}else{orderItems=order.items;}
allOrderItemsReceiptHTML="";for(var i=0;i<orderItems.length;i++){allOrderItemsReceiptHTML+=getOrderItemReceiptHTML(orderItems[i],includeNonSyncedStyling,includeOnClick,includeServerAddedText);}
...
var orderTotal=order.total;orderItems=order.items;allOrderItemsRecptHTML=getAllOrderItemsReceiptHTML(order,true,true,true,false);setReceiptsHTML(getCurrentRecptHTML()+allOrderItemsRecptHTML)
...
setStatusMessage("Order Sent");vibrate();showTablesSubscreen();manualCallHomePoll();}
...
function showPinSubscreen(){hideAllMenuSubScreens();$('#menu_screen #buttons_container').hide();$('#menu_screen #cluey_logo').show();$('#pin_screen').show();}
...
function displayDropdownSelected(selectedDisplayId){showLoadingDiv();$.ajax({type:'POST',url:'/admin/terminals/link_display',success:function(){window.location.reload();},data:{terminal_id:terminalID,display_id:selectedDisplayId}});}
...
function hideAllMenuSubScreens(){$('#menu_container').hide();$('.button[id=sales_button_'+tablesButtonID+'], .button[id=admin_screen_button_'+tablesButtonID+']').removeClass("selected");$('#table_screen').hide();$('.button[id=sales_button_'+modifyOrderItemButtonID+'], .button[id=admin_screen_button_'+tablesButtonID+']').removeClass("selected");$('#oia_subscreen').hide();$('#pin_screen').hide();}
function currentMenuSubscreenIsMenu(){return $('#menu_container').is(":visible");}
function currentMenuSubscreenIsPin(){return $('#pin_screen').is(":visible");}
...
function postSetConnectionStatus(connected){if(!connected){$('body').addClass("disconnected");if(!callHomePollInitSequenceComplete){callHomePollInitSequenceComplete=true;performCallHomePollInitSequenceComplete();}}else{$('body').removeClass("disconnected");}}
function performCallHomePollInitSequenceComplete(){$('#loading_orders_spinner').hide();hideLoadingDiv();}
...
function pinKeypadClick(val){var newVal=$('#pin_number_show').html().toString()+val;$('#pin_number_show').html(newVal);}
function doCancelPinNumberSelectKeypad(){clearMobileLoginCode();}
function doSubmitPinNumber(){if(!callHomePollInitSequenceComplete){niceAlert("Downloading Orders. Please Wait.");return;}
var entered_code=$('#pin_number_show').html();for(var i=0;i<employees.length;i++){var passcode=employees[i].passcode;if(entered_code==passcode){nickname=employees[i].nickname;id=employees[i].id
is_admin=employees[i].is_admin;mobileLoginSuccess(id,nickname,is_admin,passcode);return;}}
mobileLoginFailure();}
function mobileLoginSuccess(id,nickname,is_admin,passcode){current_user_id=id;current_user_nickname=nickname;current_user_is_admin=is_admin;current_user_passcode=passcode;storeActiveUserID(current_user_id);$.ajax({type:'POST',url:'/login',data:{employee_id:id}});$('#logged_in_user_nickname').html(current_user_nickname);showTablesSubscreen();clearMobileLoginCode();}
function mobileLoginFailure(){niceAlert("Wrong Pin Code");clearMobileLoginCode();}
function clearMobileLoginCode(){$('#pin_number_show').html("");}
function mobileLogout(){swipeToMenu();showPinSubscreen();var id_for_logout=current_user_id;current_user_id=null;storeActiveUserID(null);clearMobileLoginCode();$.ajax({type:'POST',url:'/logout',data:{employee_id:id_for_logout}});}
...
function swipeToMenu(){if(currentScreenIsMenu)return;clearAllPageFlags();doScroll((menuPageNum-1)*pageWidth);currentScreenIsMenu=true;setTimeout(function(){doMenuPageSelect(currentMenuPage,currentMenuPageId);},500);}
...
function setFirstPage(){pageNum=menuPageNum;$("#content-scroll").scrollLeft((pageNum-1)*pageWidth);currentScreenIsMenu=true;}